...
- Own the whole workflow: discover hardware; select popular systems and games; obtain authorized content; install emulators/cores; prepare and transfer files; operate folder pickers; import required BIOS/keys/firmware; configure controls, layouts and drivers; arrange/scrape the chosen frontend; launch games; verify persistence; clean temporary work.
...
- Store device records under `~/.local/state/retrofetch/handhelds/<serial>/` (outside Git); keep progress, completion evidence, and backup references together.
...
- Interrupt only at a genuine boundary: initial debugging authorization; a purchase, sign-in, legal consent or protected confirmation requiring the user; missing authorized private content; an ambiguous target; frontend selection; or an unapproved destructive change. Batch related questions and finish independent work while blocked. Do not bypass protected prompts or report blocked steps as complete.
...
6. Cold-relaunch after saving/importing, verify persistence, and launch from the chosen frontend’s actual UI to exercise URI grants. Remove only your own UI-dump/screenshot scratch files when finished.
...
- Frontend: **always ask which frontend the user wants**, including keeping the installed frontend. Present the current options below with material tradeoffs before installing or replacing one; an explicit choice already supplied for this setup satisfies the question. Never silently choose ES-DE or change Android HOME.
- User-provided material: a licensed installer for the chosen frontend if required, authorized game/BIOS/key/firmware dumps and relevant updates/DLC. Request only genuinely missing prerequisites, not information obtainable from the connected device.

### Frontend choice — ask before installing

Present a current, device-appropriate comparison, then ask the user to choose. Include price/access requirements, release channel and maintenance status, controller navigation, single/dual-screen behavior, artwork/scraping, emulator launch integration, storage/SAF support, and HOME-launcher support where verified. Explain migration costs and preserve the current frontend until the replacement works. A frontend listing a system does not mean the handheld can emulate it.

Starting directory (references checked 2026-09-05): this is a broad discovery list, **not a permanently complete catalog or popularity ranking**. Search for new projects and official release updates at each setup; verify current Android compatibility, availability and pricing rather than freezing them here.

| Frontend | Primary reference / selection considerations |
|---|---|
| ES-DE | [Project](https://www.es-de.org/) — themed, system-oriented library; detailed example below. |
| Daijishō | [Project and assets](https://github.com/TapiocaFox/Daijishou) — Android library launcher; verify current emulator player definitions. |
| Beacon Game Launcher | [Play listing](https://play.google.com/store/apps/details?id=com.radikal.gamelauncher) — compare its library/navigation workflow and supported launchers. |
| LaunchBox for Android | [Android edition](https://www.launchbox-app.com/android-about) — check Android-specific features and licensing, not desktop assumptions. |
| Pegasus | [Project](https://pegasus-frontend.org/) — customizable frontend; evaluate metadata/theme configuration effort. |
| RESET Collection | [Play listing](https://play.google.com/store/apps/details?id=com.retroloungelab.resetcollection) — compare supported emulators, scraping and controller workflow. |
| Cocoon | [Official site](https://cocoon-shell.com/) / [releases](https://github.com/inssekt/CocoonFE/releases) — evaluate its handheld-oriented interface and device/display support. |
| RetroHrai! | [Project and releases](https://github.com/retrohrai/Releases) — Android frontend with single/dual-screen support, scraping and media import; proprietary, personal-use APKs. Verify current device and emulator compatibility. |
| iiSU | [Official project](https://github.com/iisu-network/iiSU) — visuals-first handheld launcher; project identifies it as alpha at this check. Explain prerelease risks. |
| Socket | [Developer page](https://depmots.com/socket) — presentation aimed at curated collections; verify current access terms and integration coverage. |
| Stock/vendor launcher or emulator libraries | Keep the existing supported launcher, or use emulator libraries directly if the user prefers no separate frontend. |

Also investigate legacy/niche options when relevant, including DIG, ARC Browser, ATV Launcher, Console Launcher, and RetroX. Distinguish ROM frontends from general app launchers and bundled emulator services. Do not recommend an unverified APK mirror: confirm the exact developer/store listing, maintenance, device compatibility and distribution rights first. DIG's historical website/store link was unavailable during this check; do not present it as a confirmed current installation option.

Once chosen, apply the same library, artwork, permissions and launch-verification requirements to that frontend. ES-DE-specific XML, paths, themes and commands below are **conditional examples only**; use the selected frontend's documented equivalents, not ES-DE files in another app.
...
4. Inventory app versions, existing app settings, ROM/BIOS locations, frontend metadata/media, and saves. Keep private backups locally, not in the repo. Do not copy machine-specific absolute paths into the new setup.
...
- Prefer substantial samples—thousands of user ratings where available—over tiny perfect-score samples. Large samples reduce sampling uncertainty, not selection bias, review bombing, or platform/demographic bias; assess those separately. Compare confidence within the same platform/era so newer games do not crowd out older classics merely because online audiences grew. Do not blindly compare a 5-star average, a 100-point critic score, and a positive-review percentage as if they were the same measure. Record how quality, sample size, and popularity informed priority rather than presenting an unexplained synthetic score.
...
| System | Candidate emulator/core (verify chosen frontend integration) | Android package |
|---|---|---|
| Frontend | User-selected; ES-DE is one option | Discover the selected app’s official package |
...
Dolphin starting point: select a compatible Vulkan/OpenGL backend and a supported internal scale targeting the handheld’s native display viewport, following the display policy below; lower it if stable gameplay requires it. Example GameCube mapping: A/B/X/Y to labeled buttons, Z right shoulder, sticks left/right, L/R triggers. Example Wii mapping: A to A, B right trigger, 1 Y, 2 X, Minus Back, Plus Start; Nunchuk C left shoulder and Z left trigger. Adapt missing buttons and Home/menu bindings to the controller. Verify pointer and motion actions for the requested games; provide a supported alternative when sensors are absent rather than assuming a gyro exists.

### ClusterTune — compatibility-checked performance utility

Include **[ClusterTune](https://github.com/AurelioB/ClusterTune)** in the handheld provisioning workflow when compatible. Resolve the **[latest stable release](https://github.com/AurelioB/ClusterTune/releases/latest)** at setup time rather than pinning a version. The application package is `com.aure.clustertune`; it caps CPU-cluster and supported GPU maximum frequencies, not voltage or the governor.

1. Check current upstream requirements and device exceptions. Version 1.2.2 requires Android 12+ (API 31) and uses a supported built-in **PServer** backend, or an already available, user-approved **Root/su** backend. Check `adb -s SERIAL shell service list` for `PServerBinder`, then verify detection inside ClusterTune; a service name alone is not proof tuning works. Some devices are excluded from the PServer route (upstream lists Odin 2 Mini). Do not root/unlock a device just to install this utility. If neither execution method is available, report it unsupported and continue the rest of setup.
2. Download the compatible APK from the official release, verify its published SHA-256 when supplied, check package/API compatibility, and install/update through explicit-serial ADB. Open the app and inspect its execution method and detected CPU clusters/GPU controls. Record the actual version and backend in the private setup record.
3. **Installation is not permission to change clock limits.** Preserve current hardware settings and inspect the Stock profile as the recovery option. Do not activate bundled underclocks, import another device’s frequencies, assign per-app profiles, or enable boot/sleep tuning merely to prove installation. If tuning is part of the user’s requested goal, use conservative settings validated for the detected SoC, test representative games for stability/frame pacing/audio/temperature, and retain a known-working recovery path. Do not claim battery or performance gains without measurement.
4. Handle optional access deliberately: **Display over other apps** enables the quick tuner/edge picker; **Accessibility** detects foreground apps for automatic profiles; **Notifications** supports background automation; **Usage access** is optional for recent-app sorting; **Install downloaded updates** is separate from installing via ADB. Explain and obtain approval for requested special access before enabling it through Android’s UI. Do not enable Accessibility, overlays, root access, or background tuning simply because the permission dialog lists them. For installation-only requests, dismiss optional prompts and report which features remain inactive.
5. When quick controls or automatic profiles are requested, the agent operates the authorized permission/settings screens, adds the Quick Settings tile if wanted, and verifies the requested behavior. These profile controls are separate from emulator touchscreen-controller overlays. Avoid competing profile automation in other tuning apps. Cold-relaunch ClusterTune, verify backend detection and the intended profile/automation state, and leave a clear distinction between **installed**, **backend detected**, and **tuning actually tested**.
...
- Distinguish **physical panel resolution**, Android output resolution, and the emulator’s **internal rendering resolution**. Use the panel’s native output when supported; do not force an Android resolution override just to change game rendering. For 3D games, target internal rendering near the handheld panel’s native resolution for the aspect-correct game viewport, using the closest practical supported scale. For example, a 1280×960 panel displaying a 4:3 game targets approximately 1280×960; a 16:9 game on that panel uses approximately 1280×720 with letterboxing, not a stretched 1280×960 image. Emulator multipliers are relative to the emulated console, so resolve their actual dimensions rather than assuming a universal multiplier. Keep this target only while intended game speed, frame pacing, audio, thermals and battery use remain acceptable; use a lower per-game resolution when necessary. The original console’s native (1×) resolution is a diagnostic/performance fallback, not the default quality target. Record per-game exceptions and lower the scale when needed; 2× does not mean twice the handheld’s resolution. Do not force 60 FPS on a game designed for 30 FPS.
- For pixel-art systems, preserve intended aspect ratio and use integer scaling where it fits well; otherwise choose a clean aspect-correct scale without cropping important content. Letterboxing is preferable to stretching. Prefer per-game rendering/driver overrides for isolated problems; a global change needs a representative check of other affected systems and a recoverable baseline.
...
- Verify an authorized representative game through the chosen frontend into actual gameplay, with physical controls and clean rendering without diagnostic text. Use a new or disposable save slot; do not overwrite existing progress. Record measured performance only if actually measured.

## Phase 5 — Chosen frontend library and artwork

For any frontend: configure its supported ROM roots, emulator mappings, artwork and controller navigation; launch through its actual UI and cold-restart to verify persistence. Apply the following ES-DE example only if the user selected ES-DE.
...
- Launch at least one authorized representative game **from the chosen frontend** for every configured system. Record the actual emulator/core and result. Installation, successful scraping, or a ROM boot logo is not gameplay proof.
...
- Cold-restart affected emulators and the chosen frontend. Confirm settings save without permission errors, selected GPU/core persists, overlay text/buttons stay hidden, and frontend launch permissions work again.
...
- After completing the planned library expansion and verification, leave the device at its clean chosen frontend library. Stop when the selected target is met or further useful ranked choices would exceed headroom; do not endlessly restart downloads or fill the reserve.
...
**The agent is the end-to-end automation layer.** It chooses popular systems/games, installs apps/cores, operates permission/import UIs, configures emulators/controllers/drivers/layouts, prepares content, manages the chosen frontend’s metadata/artwork, and verifies gameplay. Device-specific ownership, SAF grants, controller IDs, and GPU compatibility must be discovered and handled by the agent, not handed back as routine user work. Use app-supported import/save operations rather than blindly cloning another device’s files. Keep private progress checkpoints and resume after interruptions. Only genuine user-controlled authorization, missing private material, or demonstrated technical blockers warrant intervention; never report a blocked step as automated success.

## Setup references and freshness

Use these for research, not as instructions to clone another device's settings. Check article revision dates and current official emulator documentation/release notes against the installed version. Record consulted URLs, access dates and relevant versions in the private setup record. Prefer supported official behavior when an older video or article disagrees; explain unresolved conflicts.

- [Retro Game Corps — Android Emulation Starter Guide](https://retrogamecorps.com/2022/03/13/android-emulation-starter-guide/): broad Android setup and emulator workflow reference.
- [Joey's Retro Handhelds](https://www.joeysretrohandhelds.com/): device reviews and setup guides; match advice to the actual hardware.
- [Joey's ARMSX3 setup guide](https://www.joeysretrohandhelds.com/guides/armsx3-setup-guide/): current PS2 setup reference to compare with official releases. Do not silently replace a working ARMSX2/NetherSX2 installation or assume packages, settings and frontend launch definitions are interchangeable.
- [Libretro documentation](https://docs.libretro.com/): core requirements, controls and video configuration.
- [PPSSPP documentation](https://www.ppsspp.org/docs/): PSP-specific graphics, compatibility and configuration.
- The official emulator release links in Phase 4 and frontend directory above are primary sources for supported versions and integration. Community benchmarks must identify hardware, emulator/version, renderer, resolution and tested game; a title screen or another device's FPS is not compatibility proof.

Keep game preferences and franchise requests in the private per-setup record, not this reusable guide. Refresh curation from the evidence protocol for every new setup.
